...
```sh
# Unit tests (53 tests across data, datasets, presets, train, model)
cargo test
```
...
2. `model/` — `GPT::forward` runs token embeddings → N × `Block` (RMSNorm → `CausalSelfAttention` with RoPE → RMSNorm → `MLP`) → final RMSNorm → weight-tied output projection. Attention uses Burn's fused SDPA kernel (`burn::tensor::module::attention`) with `is_causal: true` — no explicit mask tensor. `GPT::generate` uses KV caching via `GPT::forward_cached` — prefills the prompt, then decodes one token at a time reusing cached K/V tensors
...
- **Causal mask:** handled natively by the attention kernel via `is_causal: true` — no explicit mask tensor is constructed.
- **KV cache:** `GPT::forward_cached` accepts an optional `KVCache` (vec of per-layer K/V tensors). On prefill (cache=None) the full sequence is processed. On decode (cache=Some) a single token is processed and its K/V are concatenated with the cache. Position offset is derived from cache length. Causal masking is handled natively by the attention kernel (`is_causal: true`). `GPT::generate` uses this for O(seq) per-step decoding.
- **`GPT::forward`** (used for training) is unchanged — no cache, `is_causal: true` every call.

## Known gaps vs nanoGPT

- CPU fallback for top-p / combined top-k+top-p sampling (`WeightedIndex`) — fine for batch=1 inference
